...
powershell -Command "Start-Process cmd -Verb RunAs -ArgumentList '/k cd /d %~dp0 & echo \"yes\" | reg add HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\PowerShell\1\ShellIds\Microsoft.PowerShell /v \"ExecutionPolicy\" /d \"unrestricted\" & echo \"yes\" | REG ADD \"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Session Manager\Power\" /v HiberbootEnabled /t REG_DWORD /d 0 & powershell .\installer.ps1 & schtasks.exe /Create /XML DesktopModeRegistry.xml /tn DesktopModeRegistry & schtasks.exe /Create /XML GamingModeRegistry.xml /tn GamingModeRegistry & exit'"
